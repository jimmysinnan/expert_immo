...
function buildMainPrompt(data) {
  ...
  // Surfaces groupées par niveau pour la description du bâti
  const NIVEAU_ORDER = ['Sous-sol','Rez-de-chaussée','Niveau 1 (R+1)','Niveau 2 (R+2)','Combles','Extérieur / Détaché'];
  const surfByLevel = {};
  surfacesArr.filter(s => s.category === 'habitable' || !s.category).forEach(s => {
    if (!s.type) return;
    const niv = s.niveau || 'Rez-de-chaussée';
    if (!surfByLevel[niv]) surfByLevel[niv] = [];
    surfByLevel[niv].push(s.type + (s.prec ? ` (${s.prec})` : ''));
  });
  const surfByLevelText = NIVEAU_ORDER
    .filter(n => surfByLevel[n])
...

{
  ...
  "description_bati": "RESPECTER STRICTEMENT ce template — ne rien ajouter, ne rien inventer :\n\nPhrase d'ouverture OBLIGATOIRE : 'Il s'agit d'un ${formData.type_construction || '[à compléter]'} à usage d'habitation, ${formData.annee_construction ? 'édifié vers ' + formData.annee_construction + ',' : ''} élevé sur ${formData.nb_niveaux || '[niveaux à compléter]'}, distribué comme suit :'\n\nPuis, pour chaque niveau listé dans DONNÉES SURFACES ci-dessous, rédiger une puce de ce format exact :\n► NOM DU NIVEAU (en majuscules)\nUne seule phrase listant UNIQUEMENT les pièces du niveau, dans l'ordre du tableau.\n\nDATA SURFACES PAR NIVEAU (source unique, rien d'autre) :\n${surfByLevelText}\n\n${formData.notes_bien ? 'NOTES LIBRES À INTÉGRER (faits uniquement) : ' + formData.notes_bien : ''}\n\nRÈGLES ABSOLUES : (1) N'utiliser QUE les pièces listées dans DATA SURFACES — zéro invention. (2) Ne JAMAIS mentionner matériaux, accès, escaliers, ossature, revêtements — ces détails sont dans le tableau matériaux. (3) Ne pas mentionner les m². (4) Si une donnée est absente → [à rajouter par l'expert].${(formData.type_bien === 'Appartement') ? ` (5) Pour l'appartement : préciser l'étage (${formData.appart_etage || '[à compléter]'}), le type (${formData.appart_type_pieces || '[à compléter]'}), copropriété (${formData.copro_type || '[à compléter]'}, ${formData.copro_nb_batiments || '[à compléter]'} bâtiment(s), tantièmes : ${formData.copro_tantiemes || '[à compléter]'}).` : (formData.type_bien === 'Immeuble') ? ` (5) Pour l'immeuble : copropriété (${formData.copro_type || '[à compléter]'}), composition : ${formData.copro_composition || '[à compléter]'}, tantièmes : ${formData.copro_tantiemes || '[à compléter]'}.` : ''}",
...
function stripMarkdown(text) {
  return text
    ...
    .replace(/__([^_]+)__/g, '$1')       // __souligné__
    .replace(/`([^`]+)`/g, '$1');        // `code`
}

function splitParagraphs(text) {
  if (!text) return [bodyPara('[à rajouter par l\'expert]')];
  return text.split(/\n+/).filter(l => l.trim()).map(line => bodyPara(stripMarkdown(line)));
}
